...
@implementation RootViewManager {
    MainTabViewController *currentViewController;
}
...
- (instancetype)initWithPortrait:(MainTabViewController *)portrait {
    self = [super init];
    if (self) {
        self.portraitViewController = portrait;
        self.rotatingViewController = [[MainTabViewController alloc] init];
...
- (MainTabViewController *)setViewControllerForDescription:(UICompositeViewDescription *)description {
    return currentViewController;
    ...
    MainTabViewController *newMainView = description.landscapeMode ? self.rotatingViewController : self.portraitViewController;
    if (newMainView != currentViewController) {
        MainTabViewController *previousMainView = currentViewController;
...
//    [mainViewController clearCache:[TabRootViewManager instance].viewDescriptionStack];
}
...
- (void)startUp {
    @try {
        ...
        if (linphone_core_get_global_state(LC) != LinphoneGlobalOn) {
            ...
            [MainTabViewController.instance changeCurrentView:FirstLoginView.compositeViewDescription];
        } else {
...
- (UIViewController *)popCurrentView {
    ...
    if (viewStack.count <= 1) {
        ...
        LOGW(@"MainTabViewController: Trying to pop view but none stacked, going to %@!",
             DialerView.compositeViewDescription.name);
    } else {
        [viewStack removeLastObject];
        LOGI(@"MainTabViewController: Popping view %@, going to %@", currentView.name,
...
//                  transition:[MainTabViewController getBackwardTransition]
//                    animated:ANIMATED];
...
//        [viewStack removeLastObject];
//    }
//    return [self _changeCurrentView:view transition:[MainTabViewController getBackwardTransition] animated:ANIMATED];
    return nil;
}
...
//    MainTabViewController *vc = [[RootViewManager instance] setViewControllerForDescription:view];
//    if (![view equal:vc.currentView] || vc != self) {
//        LOGI(@"Change current view to %@", view.name);
//        NSMutableArray *viewStack = [RootViewManager instance].viewDescriptionStack;
//        [viewStack addObject:view];
//        if (animated && transition == nil)
//            transition = [MainTabViewController getTransition:vc.currentView new:view];
...
//    [NSNotificationCenter.defaultCenter postNotificationName:kLinMainTabViewControllerChange object:self userInfo:mdict];
//    
